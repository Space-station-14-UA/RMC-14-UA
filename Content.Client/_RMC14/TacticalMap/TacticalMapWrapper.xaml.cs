...
public sealed partial class TacticalMapWrapper : Control
{
    ...
    private Dictionary<SquadObjectiveType, string>? _squadObjectives;
    ...
    private bool IsInQueenEyeMode()
    {
        ...
        if (entMan.HasComponent<HunterTeleportingComponent>(player.Value))
            return true;
    ...
    public void UpdateSquadObjectivesFromState(Dictionary<SquadObjectiveType, string>? objectives)
    {
        _squadObjectives = objectives;
        UpdateSquadObjectives();
    }

    private void UpdateSquadObjectives()
    {
        // If no objectives dict provided, player is not in a squad - hide
        if (_squadObjectives == null)
        {
            SquadObjectivesContainer.Visible = false;
            return;
        }

        // Player is in a squad, show the container
        ...
        {
            var objectiveText = _squadObjectives.GetValueOrDefault(objectiveType, string.Empty);
            ...
            SquadObjectivesBox.AddChild(label);
        }
    }
}
