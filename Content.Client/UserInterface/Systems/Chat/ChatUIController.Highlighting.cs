...
public sealed partial class ChatUIController : IOnSystemChanged<CharacterInfoSystem>
{
    ...
    private void OnCharacterUpdated(CharacterData data)
    {
        ...
        var (_, job, _, _, jobId, entityName) = data; // Mriya
        ...
        // Mriya. Use the jobId if available, otherwise fallback to the job title. For PAI as example.
        string jobIdNonNull = jobId ?? job;
        // Convert the job title to kebab-case and use it as a key for the loc file.
        var jobKey = jobIdNonNull.Replace(' ', '-').ToLower(); // Mriya
